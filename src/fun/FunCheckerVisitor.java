...
public class FunCheckerVisitor extends AbstractParseTreeVisitor<Type> implements FunVisitor<Type> {
	...
	private void predefine() {
		...
		a.add(Type.INT);
		typeTable.put("write", new Type.Mapping(new Type.Sequence(a), Type.VOID));
	}
	...
	public Type visitProccall(FunParser.ProccallContext ctx) {
		Actual_seqContext s = ctx.actual_seq();
		if (s == null) {
			// construct an empty sequence of types
			return Sequence.EMPTY;
			
		}else {
			//  visit the result of ctx.actual_seq() to get the sequence of types
		...
		/**
		Type t = visit(ctx.actual_seq());
		if (t == null) {
			// construct an empty sequence of types
			Type.Sequence seq = new Type.Sequence(new ArrayList<>());
			
		}else {
			//  visit the result of ctx.actual_seq() to get the sequence of types
			Type seq = visit(ctx.actual_seq());
		}
	 ...
	 * @return the visitor result
	 * 
	//Type.Mapping mapping = new Type.Mapping(new Type.Sequence(array), Type.VOID);	
		return new Type.Sequence(array);
	 */
	...
	// visit every item in ctx.expr() and construct a Type.Sequence of their types.
	
	// go through the item in ctx.expr(), and visit them. By visiting them you’ll get their Type. 
	// add the them to a list, and construct a sequesnce from that list of type
	public Type visitActualSeq(FunParser.ActualseqContext ctx) {
		ArrayList<Type> array = new ArrayList<>();
		List<ExprContext> exp = ctx.expr();
		for (ExprContext e : exp) {
			Type t = visit(e);
			array.add(t);
		}
		return new Sequence(array);
	...
	public Type visitFormalseq(FunParser.FormalseqContext ctx) {
		...
		for (FunParser.Formal_declContext item : ctx.formal_decl()) {
			Type t = visit(item);
			array.add(t);
		}
		//Type.Mapping mapping = new Type.Mapping(new Type.Sequence(array), Type.VOID);
	...
	@Override
	public Type visitActualseq(ActualseqContext ctx) {
